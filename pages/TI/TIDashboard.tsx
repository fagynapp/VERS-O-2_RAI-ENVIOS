...
const TIDashboard = () => {
  const [activeTab, setActiveTab] = useState('USUARIOS'); // Default to match image focus

  // Mapeamento de Perfis (Código Interno -> Nome Amigável)
  ...
  // Mock Data using Internal Codes
  const users = [
    { id: 1, email: 'marcos.silva@policia.gov.br', nome: 'Marcos Antonio da Silva', matricula: '284.102-1', perfil: 'ADM_N3', nivel: 3, status: true, admAccess: true },
    { id: 2, email: 'ana.costa@policia.gov.br', nome: 'Ana Paula Costa', matricula: '159.330-X', perfil: 'ADM_N2', nivel: 2, status: true, admAccess: true },
    { id: 3, email: 'ricardo.lima@policia.gov.br', nome: 'Ricardo Lima Jr.', matricula: '442.115-3', perfil: 'USER', nivel: 1, status: false, admAccess: false },
    { id: 4, email: 'roberto.ti@policia.gov.br', nome: 'Roberto Tech', matricula: '999.888-7', perfil: 'TI', nivel: 4, status: true, admAccess: true },
  ];
            ...
            <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
                <div className="space-y-1.5">
                  ...
                  <input className="w-full bg-slate-50 border border-slate-200 rounded-lg px-4 py-3 text-sm text-slate-700 outline-none focus:ring-2 focus:ring-blue-500 focus:bg-white transition-all" placeholder="usuario@policia.gov.br" />
                </div>
                <div className="space-y-1.5">
                  <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Nome Completo</label>
                  <input className="w-full bg-slate-50 border border-slate-200 rounded-lg px-4 py-3 text-sm text-slate-700 outline-none focus:ring-2 focus:ring-blue-500 focus:bg-white transition-all" placeholder="Nome do Agente" />
                </div>
                <div className="space-y-1.5">
                  <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Matrícula</label>
                  <input className="w-full bg-slate-50 border border-slate-200 rounded-lg px-4 py-3 text-sm text-slate-700 outline-none focus:ring-2 focus:ring-blue-500 focus:bg-white transition-all" placeholder="000.000-0" />
                </div>
                <div className="space-y-1.5">
                  <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Perfil de Acesso</label>
                  <div className="relative">
                    <select className="w-full bg-white border border-slate-200 rounded-lg px-4 py-3 text-sm text-slate-700 outline-none focus:ring-2 focus:ring-blue-500 appearance-none cursor-pointer font-medium">
              ...
              <div className="flex items-center justify-between border-t border-slate-100 pt-6">
                ...
                <button className="bg-blue-600 hover:bg-blue-700 text-white font-bold py-2.5 px-8 rounded-lg shadow-lg shadow-blue-200 transition-all flex items-center gap-2 transform active:scale-95">
